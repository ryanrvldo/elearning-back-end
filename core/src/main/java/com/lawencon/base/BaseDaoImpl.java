...
public abstract class BaseDaoImpl<T extends Serializable> {
   ...
   * @param conditions Additional conditions with key being the field that will be searched and the
   *        value become the filter
  ...
  protected List<Object[]> getBySearch(String inquiry, List<String> selectedField,
      LinkedHashMap<String, Object> conditions, String fromTable, int page, int limit)
      throws Exception {
    ...
    StringBuilder sb = new StringBuilder("SELECT ").append(fields).append(" FROM ")
        .append(fromTable).append(" WHERE 1=1");
    ...
    if (inquiry != null && !inquiry.isEmpty()) {
      sb.append(" AND POSITION(LOWER('").append(inquiry).append("') in LOWER(CONCAT(")
          .append(fields).append("))) > 0");
    }

    List<Object[]> listResult = new ArrayList<Object[]>();

    if (page > 0 && limit > 0) {
      listResult = ConnHandler.getManager().createNativeQuery(sb.toString())
  ...
  protected List<Object[]> getBySearchV2(InquiryTemplate inquiry) {
    ...
    List<Object[]> listResult = ConnHandler.getManager().createNativeQuery(sb.toString())
        .setFirstResult(((inquiry.getPage() - 1) * inquiry.getLimit()))
        .setMaxResults(inquiry.getLimit()).getResultList();
  ...
  protected Integer getCountBySearch(String inquiry, List<String> selectedField,
    ...
    BigInteger count =
        (BigInteger) ConnHandler.getManager().createNativeQuery(sb.toString()).getSingleResult();
