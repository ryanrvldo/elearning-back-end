...
public class BaseDaoImpl<T extends Serializable> {
  ...
  protected void save(final T entity, Callback before, Callback after, boolean autoCommit)
      throws Exception {
    if (autoCommit) {
      begin();
  ...
  protected void save(final T entity, Callback before, Callback after, boolean autoCommit,
      boolean autoRollback) throws Exception {
    try {
  ...
  private void setValues(final T entity, T data, boolean isAdd) throws Exception {
    ...
    protected void delete(final T entity, Callback before, Callback after, boolean autoCommit)
        throws Exception {
      if (autoCommit) {
        begin();
    ...
    // LinkedHashMap<String, Object> conditions, String fromTable, int page, int limit) throws
    // Exception {
    ...
    //
    // StringBuilder sb = new StringBuilder("SELECT ").append(fields).append(" FROM
    // ").append(fromTable)
    // .append(" WHERE 1=1");
    //
    // for (String key : keys) {
    ...
    // sb.append(" AND POSITION(LOWER('").append(inquiry).append("') in
    // LOWER(CONCAT(").append(fields)
    // .append("))) > 0");
    // }
    //
    ...
    //
    // if (page > 0 && limit > 0) {
    // listResult = ConnHandler.getManager().createNativeQuery(sb.toString()).setFirstResult(((page
    // - 1) * limit))
    ...
    // .setFirstResult(((inquiry.getPage() - 1) *
    // inquiry.getLimit())).setMaxResults(inquiry.getLimit())
    // .getResultList();
    //
    // return listResult;
    // }
    ...
    // ConnHandler.getManager().createNativeQuery(sb.toString()).getSingleResult();
    //
    // return count.intValue();
    // }

    // protected Integer getCountBySearchV2(InquiryTemplate inquiry) {
    // StringBuilder sb = new StringBuilder("SELECT COUNT(*)").append(" FROM
    // ").append(inquiry.getViews())
    // .append(" WHERE 1=1");
    //
    // if (inquiry.isAnyCondition()) {
    // sb.append(" AND ");
    ...
    //
    // Long countData = (Long)
    /// ConnHandler.getManager().createQuery(sb.toString()).setParameter("companyId", companyId)
    // .getSingleResult();
    //
    // Integer count = countData.intValue();
    //
